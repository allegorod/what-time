// Оборачиваем весь код в DOMContentLoaded чтобы убедиться что DOM загружен
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!phoneInput || !result || !errorDiv || !flagDiv || !countryDiv || !timeDiv || !timezoneDiv) {
    ...
    phoneInput.addEventListener('input', function(e) {
        ...
        try {
            // Проверяем наличие библиотеки и находим правильную функцию парсинга
            const phoneLib = window.libphonenumber || window.libphonenumberjs || window.libphonenumberJs;
            
            if (!phoneLib) {
                console.error('Библиотека libphonenumber не загружена');
                throw new Error('Библиотека не загружена');
            }
            
            // Ищем правильную функцию парсинга
            const parseFn = phoneLib.parsePhoneNumberFromString || phoneLib.parsePhoneNumber || phoneLib.parse;
            
            if (!parseFn) {
                console.error('Функция парсинга не найдена');
                throw new Error('Функция парсинга отсутствует');
            }
            
            const phoneNumber = parseFn(value);
